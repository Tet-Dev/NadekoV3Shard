	...
	async queueSong(guildID, songLink, messageChannelBound, connection, silentAdd, mem) {
		...
		try {
			...
			while (this.handler.get(guildID).queue.length > 0){
				...
				let song = dataobj.queue.unshift();
				this.handler.set(guildID,dataobj);
				let playcon = await this.play(msg,song,msg.channel,connection,true);
				...
				if (playFinished === "end"){
					...
					if (trial < 5){
						trial++;
						if ((Date.now()/1000) - dataobj.currentSongStartTime < 10){
							connection = await this.bot.joinVoiceChannel(connection.channelID);
							playcon = await this.play(msg,song,msg.channel,connection,true);
						}
