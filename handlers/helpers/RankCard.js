...
async function composite(level, xp, next, currentFormatted, nextFormatted, colorschemeR, colorschemeG, colorschemeB, rank, avatar, bgimg, name) {
	var PromiseArray = [Jimp.read(bgimg), Jimp.loadFont("./assets/baloo/Baloo64.fnt"), Jimp.read("assets/jimpStuff/rankthing.png"), Jimp.read(avatar)];
...
if (process.argv.length == 15){
	...
	console.log(process.argv,"not right! Need 15 got"+process.argv.length);
}
