...
async function composite(level, xp, next, currentFormatted, nextFormatted, colorschemeR, colorschemeG, colorschemeB, rank, avatar, bgimg, name) {
	...
	return new Promise(async function (resolve, rej) {
		try {
			let cardGen = await spawn("node", ["./RankCard.js", level, xp, next, currentFormatted, nextFormatted, colorschemeR, colorschemeG, colorschemeB, rank, avatar, bgimg, name, "png"]);
			cardGen.stdout.on("data", async (data) => {
				await sleep(300);
				data = (data + "").replace(/\n/g, "");
				resolve({
			...
			cardGen.on("error", er => console.trace("error!" + er));
		} catch (error) {
			rej(error);
		}
...
class LevellingHandler {
	...
	async getUserData(userid, guildid, getInArr) {
		...
		if (!res || !res.length) {
			...
			res = this.getUserData(userid, guildid, true);
		}
